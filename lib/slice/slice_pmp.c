...
static int detect_region_covered_by_pmp(uintptr_t addr, uintptr_t size)
{
	...
	for (i = 0; i < n_pmp; i++) {
		...
		if ((pmp_base <= addr) && (pmp_end >= input_end)) {
			slice_printf(
				"Mem region (%lx, %lx) is already covered by pmp_%d (%lx, %lx): %lx\n",
				addr, input_end, i, pmp_base, pmp_end,
				prot_out);
			region_overlap = 1;
			break;
			...
			if (!detect_region_covered_by_pmp(
				    input_end + 1, input_end - pmp_end)) {
				return 0;
			}
			size = pmp_base - addr;
		}
			  ...
			  unsigned long addr, unsigned long size)
{
	...
	if (requested_pmp < 0) {
		slice_printf(
			"failed pmp alloc: %s:hart %d, set %d pmp[%d:%d]: mem(%lx, %lx), prot=%lx\n",
	...
	if (requested_pmp == 0) {
		slice_printf(
			"hart %d: Region (%lx, %lx) is already covered by pmp\n",
		...
		if ((exist_prot & PMP_L) == PMP_L) {
			slice_printf(
				...
				pmp_index, pmp_index + requested_pmp,
				exist_addr, exist_order, exist_prot);
			return SBI_ERR_SLICE_PMP_FAILURE;
		}
		...
		prot |= PMP_A_TOR;
		err = pmp_set_non_natural_aligned(pmp_index + 1, prot,
						  addr + size);
		if (err) {
			return err;
		}
